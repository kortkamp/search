...
import { SetUserActiveService } from '@modules/users/services/SetUserActiveService';
...
class UsersController {
  public async index(request: Request, response: Response): Promise<Response> {
    ...
    const users = await listUsers.execute({
      query: request.query as any as IListUsersDTO,
    });
  ...
  public async setActive(
    ...
    const setUserActiveService = container.resolve(SetUserActiveService);

    const { active } = request.body;
    const userId = request.params.id;
    const authUserId = request.user.id;

    const user = await setUserActiveService.execute({
      authUserId,
      active,
      userId,
    });
